...
function DayCellImpl({ day, completedCount, totalCount, color, isSelected, onSelect }: DayCellProps) {
  ...
  if (!day.isCurrentMonth) {
    return (
      <div
        aria-hidden
        className="flex aspect-square min-h-11 w-full items-start justify-start rounded-lg p-1.5 text-xs text-muted-foreground/30 sm:items-center sm:justify-center"
      >
        {dayNumber}
      </div>
    )
  ...
  return (
    <motion.button
      ...
      className={cn(
        'relative flex aspect-square min-h-11 w-full flex-col items-start justify-start rounded-lg p-1.5 text-left text-xs font-medium transition-colors sm:items-center sm:justify-center sm:text-center',
