...
function DayCellImpl({ day, completedCount, totalCount, color, isSelected, onSelect }: DayCellProps) {
  ...
  if (!day.isCurrentMonth) {
    return (
      <div
        aria-hidden
        className="flex aspect-square min-h-11 w-full items-center justify-center rounded-lg p-1.5 text-xs text-muted-foreground/30"
      >
        {dayNumber}
      </div>
    )
  ...
  return (
    <motion.button
      ...
      className={cn(
        'relative flex aspect-square min-h-11 w-full flex-col items-center justify-center rounded-lg p-1.5 text-center text-xs font-medium transition-colors',
